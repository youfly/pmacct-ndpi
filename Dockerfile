...
RUN git clone --depth 1 https://github.com/ntop/nDPI.git /tmp/nDPI && \
    cd /tmp/nDPI && ./autogen.sh && make -j$(nproc) && make install && ldconfig
...
RUN git clone --depth 1 https://github.com/pmacct/pmacct.git /tmp/pmacct && \
    cd /tmp/pmacct && ./autogen.sh && \
    ./configure --enable-ndpi --enable-sqlite3 --enable-jemalloc --enable-json --prefix=/usr/local && \
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    libpcap0.8 libsqlite3-0 libjson-c5 libmnl0 libnuma1 libjemalloc2 \
